...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', sans-serif; }
        ...
        .menu-item { background: #1e1b4b; border: 1px solid rgba(255,255,255,0.06); padding: 14px; border-radius: 14px; display: flex; justify-content: space-between; align-items: center; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); animation: fadeUp 0.4s ease-out forwards; opacity: 0; }
        .menu-item:hover { transform: translateY(-2px); border-color: rgba(99,102,241,0.4); box-shadow: 0 8px 25px rgba(0,0,0,0.3); }
        @keyframes fadeUp {
        ...
        .btn-qty { width: 32px; height: 32px; border-radius: 10px; border: none; background: rgba(255,255,255,0.08); color: white; font-size: 18px; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); }
        .btn-qty:not(:disabled):hover { background: rgba(255,255,255,0.15); }
        .btn-qty:not(:disabled):active { transform: scale(0.85); background: rgba(255,255,255,0.25); }
        ...
        .btn-submit { width: 100%; height: 50px; border-radius: 14px; border: none; background: linear-gradient(135deg, #22c55e, #16a34a, #22c55e); background-size: 200% auto; color: white; font-size: 16px; font-weight: 700; cursor: pointer; box-shadow: 0 4px 15px rgba(34,197,94,0.3); display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .btn-submit:not(:disabled):hover { background-position: right center; box-shadow: 0 8px 25px rgba(34,197,94,0.4); transform: translateY(-2px); }
        .btn-submit:not(:disabled):active { transform: translateY(0) scale(0.97); }
        .btn-submit:disabled { opacity: 0.5; cursor: not-allowed; box-shadow: none; background: #374151; }
        
        .breathing-btn:not(:disabled) { animation: breathing 2s infinite ease-in-out; }
        @keyframes breathing {
            0%, 100% { transform: scale(1); box-shadow: 0 4px 15px rgba(34,197,94,0.3); }
            50% { transform: scale(1.02); box-shadow: 0 8px 25px rgba(34,197,94,0.5); }
        }
        ...
        .promo-image { display: block; width: 100%; border-radius: 24px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05); box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
...
<body>
    ...
    <style>
        ...
        .cat-btn { flex-shrink: 0; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); color: #9ca3af; padding: 8px 16px; border-radius: 20px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); white-space: nowrap; }
        .cat-btn:hover { background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.2); }
        .cat-btn.active { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; border-color: transparent; box-shadow: 0 4px 15px rgba(99,102,241,0.4); transform: scale(1.03); }
    </style>
    ...
    <div style="margin-top: 24px; padding: 14px; background: #1e1b4b; border-radius: 14px; border: 1px solid rgba(255,255,255,0.06); margin-bottom: 12px;">
        <div style="font-size: 14px; font-weight: 600; color: #86efac; margin-bottom: 8px;">👤 Tên người nhận (Bắt buộc):</div>
        <input type="text" id="order-name" placeholder="Nhập tên của bạn" style="width: 100%; height: 44px; background: rgba(0,0,0,0.25); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: white; padding: 0 12px; font-size: 14px; outline: none;">
    </div>
    
    <div style="margin-top: 0px; padding: 14px; background: #1e1b4b; border-radius: 14px; border: 1px solid rgba(255,255,255,0.06); margin-bottom: 24px;">
    ...
    <div class="success-overlay" id="notify-overlay" style="display: none; background: rgba(13,10,33,0.95); z-index: 100000;">
        <div class="success-icon" id="notify-icon" style="font-size: 40px; margin-bottom: 12px;">🔔</div>
        <h2 style="font-weight: 800; color: #60a5fa; font-size: 22px;" id="notify-title">Thông báo</h2>
        <p style="color: #e5e7eb; font-size: 15px; margin-top: 8px;" id="notify-msg"></p>
        <button class="btn-submit" style="margin-top: 24px; max-width: 200px; background: rgba(255,255,255,0.1);" id="notify-btn" onclick="closeNotify()">Đóng</button>
    </div>
    ...
    <script>
        ...
        function updateTotal() {
            ...
            var btnSubmit = document.getElementById("submit-order-btn");
            btnSubmit.disabled = (totalQty === 0);
            if (totalQty > 0) {
                btnSubmit.classList.add("breathing-btn");
            } else {
                btnSubmit.classList.remove("breathing-btn");
            }
        }

        function submitOrder() {
            var itemsList = [];
            var allAreService = true;
            menuItems.forEach(function(item) {
            ...
            var customerName = document.getElementById("order-name").value.trim();
            if (!allAreService && !customerName) {
                alert("Vui lòng nhập tên (bắt buộc khi gọi đồ ăn/thức uống)!");
            ...
            if (phone && !phoneRegex.test(phone)) {
                alert("Số điện thoại không hợp lệ. Vui lòng nhập đúng định dạng hoặc để trống!");
            ...
            var finalNote = "";
            if (customerName) finalNote += "Tên khách: " + customerName;
            if (note) finalNote += (finalNote ? " - " : "") + "Ghi chú: " + note;
            ...
            xhr.send(JSON.stringify({ items: itemsList, note: finalNote, phone: phone }));
        }
        ...
        function pollClientMsgs() {
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "/api/client-poll/" + tableId + "?_=" + Date.now(), true);
            xhr.onload = function() {
                if (xhr.status === 200) {
                    var res = JSON.parse(xhr.responseText);
                    if (res.has_message) {
                        var m = res.data;
                        document.getElementById("notify-msg").textContent = m.message;
                        if (m.type === "error") {
                            document.getElementById("notify-icon").textContent = "❌";
                            document.getElementById("notify-title").style.color = "#f87171";
                            document.getElementById("notify-title").textContent = "Từ chối";
                        } else {
                            document.getElementById("notify-icon").textContent = "✅";
                            document.getElementById("notify-title").style.color = "#34d399";
                            document.getElementById("notify-title").textContent = "Thành công";
                        }
                        
                        var btn = document.getElementById("notify-btn");
                        if (m.type === "redirect" && m.redirect_url) {
                            btn.textContent = "Chuyển sang bàn mới";
                            btn.style.background = "linear-gradient(135deg, #10b981, #059669)";
                            btn.onclick = function() { window.location.href = m.redirect_url; };
                        } else {
        ...
        setInterval(pollClientMsgs, 3000);
    </script>
</body>
</html>"""
...
async def customer_menu(table_id: int):
    ...
    try:
        ...
        db_products.extend([
            ...
            { "name": "Đổi sang bàn Líp", "price": 0, "category": "Yêu cầu nghiệp vụ" },
            { "name": "Đổi sang bàn 3 Băng", "price": 0, "category": "Yêu cầu nghiệp vụ" },
            { "name": "Đổi sang bàn Lỗ", "price": 0, "category": "Yêu cầu nghiệp vụ" },
...
async def customer_order(table_id: int, payload: dict):
    ...
    try:
        ...
        for item in items:
            ...
            is_service = price == 0 and (item_name in ["Lấy lơ", "Xếp bi", "Quét bàn"] or item_name.startswith("Đổi sang bàn") or item_name.startswith("Yêu cầu khác"))
            if is_service:
...
async def get_history_sessions():
    """Lấy danh sách các phiên chơi đã hoàn tất"""
    db = SessionLocal()
    try:
        ...
        for s in sessions:
            ...
            # Kiem tra neu qua 48h (can_delete = True)
            can_delete = False
            if s.end_time and (now - s.end_time) > timedelta(hours=48):
                can_delete = True
                
            items = db.query(SessionOrderItem).filter(SessionOrderItem.session_id == s.id).all()
            items_list = [{"item_name": item.item_name, "quantity": item.quantity, "price": item.price, "total_price": item.total_price} for item in items]
                
            result.append({
                "id": s.id,
                "table_name": table.name if table else f"Ban {s.table_id}",
                "start_time": s.start_time.isoformat() + "Z",
                "end_time": s.end_time.isoformat() + "Z" if s.end_time else None,
                "total_minutes": s.total_minutes,
                "play_fee": s.play_fee,
                "items": items_list,
                "total_bill": s.play_fee + sum([item.total_price for item in items]),
...
async def delete_history_sessions(payload: dict):
    """Xoa cac phien choi qua 48h"""
    session_ids = payload.get("session_ids", [])
    if not session_ids:
        return JSONResponse({"status": "error", "message": "Danh sach id trong"}, status_code=400)
    ...
    try:
        ...
        for s_id in session_ids:
            session = db.query(PlaySession).filter(PlaySession.id == s_id).first()
            if session and session.status == "COMPLETED" and session.end_time:
                if (now - session.end_time) > timedelta(hours=48):
                    db.delete(session)
                    deleted_count += 1
        db.commit()
        return JSONResponse({"status": "ok", "message": f"Da xoa {deleted_count} phien choi"})
    except Exception as e:
        return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    finally:
        db.close()
...
async def admin_add_table(payload: dict):
    ...
    if not name:
        return JSONResponse({"status": "error", "message": "Ten ban khong duoc de trong"}, status_code=400)
...
@app.post("/api/admin/tables/update")
...
@app.post("/api/client-notify/{table_id}")
...
@app.get("/api/client-poll/{table_id}")
async def poll_client(table_id: int):
    r = redis_lib.Redis(host='localhost', port=6379, db=0)
    msg = r.get(f"client_msg_{table_id}")
    if msg:
        r.delete(f"client_msg_{table_id}")
        return JSONResponse({"has_message": True, "data": json.loads(msg)})
    return JSONResponse({"has_message": False})

@app.post("/api/session/transfer/{from_table_id}/{to_table_id}")
async def transfer_session(from_table_id: int, to_table_id: int):
    """Chuyen ban dang choi sang ban trong"""
    db = SessionLocal()
    try:
        if from_table_id == to_table_id:
            return JSONResponse({"status": "error", "message": "Bàn gốc và bàn đích giống nhau!"}, status_code=400)
        ...
        if from_table.current_status != "PLAYING":
            return JSONResponse({"status": "error", "message": "Bàn gốc chưa được bắt đầu tính giờ!"}, status_code=400)
            
        if to_table.current_status != "EMPTY":
            return JSONResponse({"status": "error", "message": "Bàn đích đang có khách, không thể chuyển!"}, status_code=400)
        ...
        if not active_session:
            return JSONResponse({"status": "error", "message": "Bàn gốc không có phiên chơi nào đang hoạt động!"}, status_code=400)
        ...
        event_data = {
            ...
            "event_type": "TABLE_ACTIVE",
            "message": f"Nhân viên đã chuyển khách từ {from_table.name} sang {to_table.name}."
        }
        r.set('latest_ai_event', json.dumps(event_data))

        client_data = {
            "message": f"Yêu cầu đổi bàn được chấp nhận! Bạn đã được chuyển sang {to_table.name}.",
            "type": "redirect",
            "redirect_url": f"/menu/{to_table.id}",
            "timestamp": time.time()
        }
        r.set(f"client_msg_{from_table_id}", json.dumps(client_data))
        r.expire(f"client_msg_{from_table_id}", 300)
        
        return JSONResponse({"status": "ok", "message": f"Đã chuyển thành công sang {to_table.name}!"})
    except Exception as e:
        return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    finally:
        db.close()
